...
:idprefix:
:idseparator: -
:application_styles: ./application-styles.adoc
:primeng_scoping: ./primeng-integration.adoc
...
|Contains CSS for the whole application recognized by the scope identifier. See link:{application_styles}[Application Styles] for lifecycle details.
...
[#primeng]
=== Purpose - PrimeNG
PrimeNG style sheets are constructed by the PrimeNG library and modified by OneCX, and in the case of theme overrides, style sheets are constructed by OneCX completely. See link:{primeng_scoping}[PrimeNG Integration] for details.
